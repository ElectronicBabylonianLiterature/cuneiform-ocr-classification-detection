...
def parse_args():
    ...
    parser.add_argument(
        "--img-out-dir", type=str, default="", help="Output directory of images."
    )
    parser.add_argument(
        "--det", type=str, default=None, help="Pretrained text detection algorithm"
    )
    parser.add_argument(
        "--det-config",
        type=str,
    ...
    parser.add_argument(
        "--recog", type=str, default=None, help="Pretrained text recognition algorithm"
    )
    parser.add_argument(
        "--recog-config",
        type=str,
    ...
    if not os.path.samefile(args.config_dir, os.path.join(str(Path.cwd()))) and (
        ...
        warnings.warn(
            "config_dir will be overridden by det-config or recog-config.", UserWarning
        )
    return args
...
class MMOCR:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        inferencer_kwargs.update(
            self._get_inferencer_kwargs(recog, recog_config, recog_ckpt, "rec_")
        )
        inferencer_kwargs.update(
            self._get_inferencer_kwargs(kie, kie_config, kie_ckpt, "kie_")
        )
        self.inferencer = MMOCRInferencer(det_config, det_ckpt, device=device)

    def _get_inferencer_kwargs(
        self,
        model: Optional[str],
        config: Optional[str],
        ckpt: Optional[str],
        ...
        if model is not None:
            ...
            kwargs[prefix + "config"] = os.path.join(self.config_dir, cfgs["config"])
            kwargs[prefix + "ckpt"] = (
                "https://download.openmmlab.com/" + f'mmocr/{cfgs["ckpt"]}'
            )

        if config is not None:
            if kwargs.get(prefix + "config", None) is not None:
                warnings.warn(
                    f"{model}'s default config is overridden by {config}", UserWarning
                )
        ...
        if ckpt is not None:
            if kwargs.get(prefix + "ckpt", None) is not None:
                warnings.warn(
                    f"{model}'s default checkpoint is overridden by {ckpt}", UserWarning
                )
    ...
    def get_model_config(self, model_name: str) -> Dict:
        ...
        model_dict = {
            ...
            "DRRG": {
                "config": "textdet/" "drrg/drrg_resnet50_fpn-unet_1200e_ctw1500.py",
                "ckpt": "textdet/"
                "drrg/"
            ...
            "FCE_IC15": {
                "config": "textdet/" "fcenet/fcenet_resnet50_fpn_1500e_icdar2015.py",
                "ckpt": "textdet/"
                "fcenet/"
            ...
            "MaskRCNN_CTW": {
                "config": "textdet/" "maskrcnn/mask-rcnn_resnet50_fpn_160e_ctw1500.py",
                "ckpt": "textdet/"
                "maskrcnn/"
            ...
            # },
            "PANet_CTW": {
                "config": "textdet/" "panet/panet_resnet18_fpem-ffm_600e_ctw1500.py",
                "ckpt": "textdet/"
                "panet/"
            ...
            "PANet_IC15": {
                "config": "textdet/" "panet/panet_resnet18_fpem-ffm_600e_icdar2015.py",
                "ckpt": "textdet/"
                "panet/"
            ...
            "PS_CTW": {
                "config": "textdet/" "psenet/psenet_resnet50_fpnf_600e_ctw1500.py",
                "ckpt": "textdet/"
                "psenet/"
            ...
            "PS_IC15": {
                "config": "textdet/" "psenet/psenet_resnet50_fpnf_600e_icdar2015.py",
                "ckpt": "textdet/"
                "psenet/"
            ...
            "SAR": {
                "config": "textrecog/sar/"
                "sar_resnet31_parallel-decoder_5e_st-sub_mj-sub_sa_real.py",
                "ckpt": "textrecog/sar/sar_resnet31_parallel-decoder_5e_st-sub_mj-sub_sa_real/sar_resnet31_parallel-decoder_5e_st-sub_mj-sub_sa_real_20220915_171910-04eb4e75.pth",  # noqa: E501
            },
            # 'SAR_CN': {
            #     'config':
            #     'textrecog/'
            ...
            "ASTER": {
                "config": "textrecog/aster/aster_resnet45_6e_st_mj.py",
                "ckpt": "textrecog/"
